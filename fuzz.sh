...
if [ ! -f ./markdown-fuzz.zip ]; then
    mkdir -p autoeq/testdata/fuzz-workdir/corpus
    cp autoeq/testdata/*.txt autoeq/testdata/fuzz-workdir/corpus/
    echo "running go-fuzz-build, might take a while..."
    go-fuzz-build github.com/indiependente/autoEqMac/autoeq
fi

echo "running go-fuzz"
go-fuzz -bin=./markdown-fuzz.zip -workdir=autoeq/testdata/fuzz-workdir
